...
subtitle overlay, web controls (the MJPEG preview from #376 IS implemented:
`--web` serves `/api/preview.jpg` and `/api/preview.mjpeg` on port 8080),
config file loading, systemd unit (`config/subtitler.example.json` and
`config/subtidlerd.service` are both empty placeholders). Current code is the
low-latency passthrough plus a pink no-signal fallback: when the capture side
stops, the frame buffer is kept stocked with generated pink frames and output
keeps running.
...
- `src/web/` builds the static lib `web` (alias `subtitler::web`): the appliance web server (`WebServer`, libsoup 3) with the MJPEG preview endpoints (#382). It reads the shared preview frame buffer and toggles the preview gate via an activation callback; it must not depend on `stream/` or Gst.
- `src/utils/` is a header-only INTERFACE lib `utils` (alias `subtitler::utils`) with shared utilities: `reset_guard.h` (`ResetGuard`), `unique_ptr.h` (`FunctionDeleter`/`UniquePtr`, RAII for C APIs that release via a free function), and `preview_frame.h` (`PreviewFrameBuffer`, the shared latest-JPEG buffer between the stream and web modules). Utils must stay dependency-free — no Gst, no other modules. Anything Gst-dependent belongs in `stream/` (e.g. `deleters.h`); anything used by only one module stays local to that module.
...
- Output pipeline clock and latency are **pinned** in `StartOutput` (`gst_pipeline_use_clock` to the system clock, `gst_pipeline_set_latency` to `kTargetLatency`). Both pins are load-bearing once the alsasink audio branch exists: without the clock pin the pipeline adopts the audio ring-buffer clock (starts at zero when the device starts, breaking the timestamp anchors), and without the latency pin GStreamer configures the alsasink's reported latency (hundreds of ms) pipeline-wide — sinks render at PTS + latency, so the render schedule then demands more in-flight frames than the buffers hold and the frame buffer starves permanently (observed as ~15 fps output and constant frame drops, #128). The `OutputAnchor` timestamp mapping subtracts the configured latency from each buffer's head start, keeping the end-to-end delay at `kTargetLatency` (3 frames) regardless of sink-reported latency.
- Usage: `subtitler [video-device] [connector-id] [--output=software|pisp|window|null] [--no-audio] [--audio-output-device=<alsa-device>] [--audio-offset=<ms>] [--web]` (`--audio-offset` shifts audio relative to video: positive delays audio, negative advances it — realized by delaying the counter-stream so no render deadline falls before buffer arrival; the #130 sync knob). Defaults: `/dev/video0`, auto connector, software NV16 output, CV105 audio capture on, vc4-hdmi audio output auto-detected as the first openable vc4hdmi port via `plughw:` — raw `hw:` can't work, the MAI DAI accepts only IEC958 subframes, see docs/pi-setup.md). `--web` starts the web server on port 8080 and builds the MJPEG preview branch.
