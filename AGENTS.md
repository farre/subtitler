...
- `src/utils/` is a header-only INTERFACE lib `utils` (alias `subtitler::utils`) with shared utilities: `reset_guard.h` (`ResetGuard`) and `unique_ptr.h` (`FunctionDeleter`/`UniquePtr`, RAII for C APIs that release via a free function). Utils must stay dependency-free — no Gst, no other modules. Anything Gst-dependent belongs in `stream/` (e.g. `deleters.h`); anything used by only one module stays local to that module.
