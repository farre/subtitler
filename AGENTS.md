...
- Usage: `subtitler [video-device] [connector-id] [--output=software|pisp|window|null] [--no-audio] [--audio-output-device=<alsa-device>]` (defaults: `/dev/video0`, auto connector, software NV16 output, CV105 audio capture on, vc4-hdmi audio output at `hw:CARD=vc4hdmi0,DEV=0` — the card id is pending verification in #127).
