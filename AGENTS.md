...
`--api-key` value as JSON. Also implemented: the persistent
...
Planned but NOT yet implemented: the #15 web interface proper (static
assets in `web/` — `index.html` + `assets/style.css`, growing
piecemeal) and the systemd unit (`config/subtidlerd.service` is an
empty placeholder).
...
- Installation covers the `subtitler` and `subtitler-probe` binaries (`install(TARGETS ...)` + GNUInstallDirs in the root CMakeLists), the `web/` assets (`install(DIRECTORY web ...)` to `${CMAKE_INSTALL_DATADIR}/subtitler`), and the example configuration (`config/subtitler.example.ini` to the same data dir). When `config/subtidlerd.service` gets implemented, add an install rule for it too.
