...
`/api/preview.mjpeg` on port 8080), and the SRT subtitle overlay from #438
(`--subtitles=<file>`, plus boot resume of the state dir's `active` marker).
Planned but NOT yet implemented: web subtitle upload/sync controls (the POST
side of #438), config file loading, systemd unit
